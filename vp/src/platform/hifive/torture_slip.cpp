...
TortureSLIP::TortureSLIP(sc_core::sc_module_name, uint32_t irqsrc, SymbolicContext &_ctx,
  ...
  : solver(_ctx.solver), ctx(_ctx.ctx) {
	...
	this->create_input(upper, pktsiz);
	tsock.register_b_transport(this, &TortureSLIP::transport);
}

TortureSLIP::~TortureSLIP(void) {
	return;
}

void TortureSLIP::push_byte(uint8_t byte) {
	switch (byte) {
	case SLIP_END:
		input.push(esc);
		input.push(esc_end);
...
void TortureSLIP::create_input(std::ifstream *stream, size_t pktsiz) {
	if (stream) {
		char c;
...
uint32_t *TortureSLIP::addr2register(uint64_t addr) {
	switch (addr) {
...
void TortureSLIP::transport(tlm::tlm_generic_payload &trans, sc_core::sc_time &delay) {
	...
	if (cmd == tlm::TLM_READ_COMMAND) {
		...
		case RXDATA_REG_ADDR:
			...
			if (input.empty() || !(ie & UART_RXWM)) {
				...
				rxdata = solver.evalValue<uint32_t>(value->concrete);
